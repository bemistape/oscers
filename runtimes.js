...
// 1) CSV URLs for game data & reactions
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  try {
    // 3) Load game data + reactions
    await fetchMainCSV();
    await fetchReactionsCSV();

    // 4) Enable the "Begin" button
  ...
  document.getElementById("submit-score-btn").addEventListener("click", () => {
    submitScoreToAirtable(playerName, new Date().toISOString(), totalScore, difficulty);
  });

  // 7) Round recap overlay
  ...
  // 8) Load scoreboard from Airtable
  loadScoreboardFromAirtable();
});

/*******************************************************
 * LOAD MAIN CSV & REACTIONS
 *******************************************************/
...
function parseMainCSV(csvData){
  ...
  // columns: id, movie_name, runtime, difficulty, group, poster, overview
 ...
 *******************************************************/
/**
 * Called when user clicks "Submit Score" on final screen.
 */
function submitScoreToAirtable(name, timestamp, score, difficulty){
  // We'll POST a record to Airtable with these fields
  const postBody = {
    records: [
      {
        fields: {
          ...
          timestamp,
          difficulty,
          score: parseFloat(score) // ensure numeric
        }
      }
    ]
  };
  ...
  .then(resp => resp.json())
  .then(data => {
    console.log("Airtable POST response:", data);
    if(data.records){
      alert("Score submitted successfully to Airtable!");
    } else {
      alert("Airtable error: " + JSON.stringify(data));
    }
  })
  .catch(err=>{
    console.error("Error submitting score to Airtable:", err);
    alert("Error submitting score to Airtable. Check console for details.");
  });
}
...
async function loadScoreboardFromAirtable(){
  try {
    // We do a GET on the table to retrieve all records
    // You can append ?view=viwiuSoH95w01Kk7c if you want a specific view
    const url = AIRTABLE_TABLE_URL; 
    ...
    // data.records is an array of { id, fields: { name, timestamp, difficulty, score }, createdTime }
    ...
    data.records.forEach(rec => {
      ...
      switch(f.difficulty.toLowerCase()){
        ...
        case "hard":
          ...
          break;
        default:
          // ignore or store in an "unknown" bucket
          break;
      }
    });
    ...
    // Remove "Loading scoreboard..." placeholder if present
